...
class IdleRPGApp:
    ...
    def make_wrapping_label(self, parent, text, **kwargs):
        ...
        lbl.bind('<Configure>', lambda e: lbl.config(wraplength=e.width - 20))
        return lbl
    ...
    def set_background(self, parent, bg_type):
        ...
        if bg_type in self.bg_images:
            self.bg_label.config(image=self.bg_images[bg_type])
    ...
    def delete_save_menu(self, saves):
        ...
        ttk.Button(win, text="Usuń Wybrany", style="Danger.TButton", command=del_selected).pack(pady=5)
        ttk.Button(win, text="Wyczyść Wszystko", style="Danger.TButton", command=del_all).pack(pady=5)
        ttk.Button(win, text="Zamknij", style="Fantasy.TButton", command=win.destroy).pack(pady=5)
    ...
    def build_main_ui(self):
        ...
        log_frame = tk.Frame(main_area, height=130, bd=5, relief=tk.SUNKEN, bg="#000000")
...
Zrekrutowano: {unlocked_count}/6
    ...
    def show_expedition(self):
        ...
        for enemy in choices:
            ...
            btn = tk.Button(card, text="WALCZ", font=("Georgia", 16, "bold"), bg="#3e2723", fg="#f4d03f", 
                            activebackground="#5d4037", activeforeground="#f7dc6f", bd=5, relief=tk.RAISED,
                            command=lambda e=enemy: self.select_enemy(e))
            btn.pack(side=tk.BOTTOM, pady=20, ipadx=40, ipady=10)

        refresh_btn = tk.Button(self.view_panel, text="🔄 Odśwież Przeciwników", font=("Georgia", 16, "bold"), bg="#3e2723", fg="#f4d03f", 
                        activebackground="#5d4037", activeforeground="#f7dc6f", bd=5, relief=tk.RAISED,
                        command=self.show_expedition, padx=20, pady=5)
    ...
    def show_equipment(self, selected_item_dict=None, is_equipped_slot=None):
        ...
        def on_drag_start(event, item_dict):
            ...
            self.drag_phantom.config(bg="#111")
    ...
    def open_debug_console(self):
        ...
        def set_custom_item():
            ...
            lbl_status.config(text=f"Dodano przedmiot {item_id} do ekwipunku!")
            
        tk.Button(f_item, text="Dodaj", bg="#5d4037", fg="white", command=set_custom_item).pack(side=tk.LEFT)
